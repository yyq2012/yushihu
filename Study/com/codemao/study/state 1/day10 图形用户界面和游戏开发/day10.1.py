...
# 在此也无需进行赘述。Python默认的GUI开发模块是tkinter
# （在Python 3以前的版本中名为Tkinter），
...
def main():
    ...
    top.title('小游戏')
    # 创建标签对象并添加到顶层窗口
    label = tkinter.Label(top, text='Hello, yu_shi_hu!', font='Arial -32', fg='red')
    label.pack(expand=1)
    # 创建一个装按钮的容器
...
# 所以需要这样的一个循环一直运行着等待下一个事件的发生。
# 另一方面，Tk为控件的摆放提供了三种布局管理器，
# 通过布局管理器可以对控件进行定位，
# 这三种布局管理器分别是：Placer（开发者提供控件的大小和摆放位置）、
# Packer（自动将控件填充到合适的位置）和Grid（基于网格坐标来摆放控件），
# 此处不进行赘述。
